<?php	
	$con = mysqli_connect("localhost:3306","root","PROJECT","Basic");
	if (mysqli_connect_errno())
	{
		die('Could not connect');
	}
	
	$Hotel_Group_id				 =	 $_GET['Hotel_Group_id'];
	$Email					 =	 $_GET['Email'];	

	if (	$Email				===		'' 
		or $Hotel_Group_id		===		''	)

		{
			header('Location:Email-Insert-Hg-Make.php');
			exit;
		}
	
	$result = mysqli_query($con, "INSERT INTO `Hotel_Group_Email_Address`(`Hotel_Group_id`,`Email`) VALUES ('$Hotel_Group_id','$Email')");
		
	if($result === FALSE) {
		die(mysqli_error($con)); 
	}
	mysqli_close($con);
?>	

<html style="background-image:url(../Images/background.jpg);">

	<head>
		<title> DB 2017-18 </title>
		<meta charset="UTF-8">
		<link type="text/css" rel="stylesheet" href="../CSS/stylesheet_general.css"/>
		<link rel="icon" href="../Images/favicon.png"/>
		
	</head>
	
	<body>
		<meta http-equiv="refresh" content="2; ../After-Insert.html" />
		<h1 align="center" style="font-family:Segoe UI;">
			<br><br>
			<strong>Redirecting you in 3 seconds...</strong>
		</h1>
	</body>
</html>
